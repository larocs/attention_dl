...
QUERY_URL = 'https://arxiv.org/search/advanced?advanced=&terms-0-operator=AND&terms-0-term=%22dynamic+neural+network%22&terms-0-field=all&terms-1-operator=OR&terms-1-term=%22adaptive+neural+network%22&terms-1-field=all&terms-2-operator=OR&terms-2-term=%22conditional+computation%22+%22neural+network%22&terms-2-field=all&terms-3-operator=OR&terms-3-term=%22gated+neural+network%22&terms-3-field=all&terms-4-operator=OR&terms-4-term=%22adaptive+inference%22+%22neural+network%22&terms-4-field=all&terms-5-operator=OR&terms-5-term=%22routing+neural+network%22&terms-5-field=all&terms-6-operator=OR&terms-6-term=%22mixture+of+experts%22+%22neural+network%22&terms-6-field=all&terms-7-operator=OR&terms-7-term=%22merging+of+experts%22+%22neural+network%22&terms-7-field=all&terms-8-operator=OR&terms-8-term=%22early+exit%22+%22neural+network%22&terms-8-field=all&classification-computer_science=y&classification-physics_archives=all&classification-include_cross_list=include&date-filter_by=all_dates&date-year=&date-from_date=&date-to_date=&date-date_type=submitted_date&abstracts=show&size=100&order=-announced_date_first'
###
...
def save_json(path, dct):
    ...
    path = path_parts[0] + "\\" + path_parts[1].replace("/", "_")
